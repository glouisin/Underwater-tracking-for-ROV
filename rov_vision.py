...
#   - MiDaS    → estimates depth (the z-axis) from a single RGB image
#
# Detected objects are tracked across frames using a simple centroid tracker.
# The operator selects a target from the Mac interface; the ROV then streams
...
# MiDaS depth model quantized to W8A8 (8-bit weights and activations)
...
MAC_IP           = "192.168.5.45" # IP for streaming
UDP_IP           = "127.0.0.1"
...
FRAME_H       = 720
FPS_OUT       = 80    # GStreamer stream framerate (can exceed camera capture rate)
CONF_THRES    = 0.25  # detections below this confidence score are discarded

MIN_CY_VALID  = 60      # NOUVEAU : rejette les détections trop hautes dans l'image (fantômes de surface)

NMS_IOU_THRES = 0.50  # IoU threshold for Non-Maximum Suppression — lower = keep more overlapping boxes
#EMA parameters
...
# -------------------- Load MiDaS --------------------
# MiDaS outputs a *relative* inverse-depth map: higher raw values mean the
# pixel is closer to the camera. We later invert this convention so that
# a higher z_corrected value corresponds to a farther object.
...
# into MiDaS's smaller input grid to sample the depth at the right location.
scale_x_midas = in_w_midas / FRAME_W
scale_y_midas = in_h_midas / FRAME_H
Z_MAX = 255.0 * out_scale_midas  # theoretical maximum depth value after dequantization
...
pipeline = Gst.parse_launch(
    f'appsrc name=src '
    f'is-live=true '
    f'block=false ' # put to false for optimization
    f'format=time '
...
while True:
    ...
    # MiDaS — RGB
    # MiDaS was trained on RGB images, so we convert from OpenCV's default BGR.
    # Quantization here targets UINT8 [0,255] instead of INT8, hence different clip bounds.
    ...
    run_midas = selected_id is not None
    if run_midas: # correction
    ...
    if len(boxes):
        ...
        if len(idx_cv2):
            ...
            for local_idx, i in enumerate(indx):
                ...
                # MiDaS depth map's coordinate system, then apply EMA smoothing
                # separately on the MiDaS coordinates to reduce quantisation noise
                # before we sample the depth value.
                ...
                # EMA on depth: very low alpha (0.05) because depth from MiDaS
                # is particularly noisy — we accept slow reaction in exchange
