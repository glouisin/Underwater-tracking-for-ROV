...
IOU_TRACK_THRESH = 0.1 # minimum overlap to consider same object
# -------------------- Underwater filtering parameters --------------------
# Balance des blancs : décalage manuel sur les axes a* et b* (LAB)
# A_SHIFT > 0  → pousse vers le rouge  |  A_SHIFT < 0 → pousse vers le vert
# B_SHIFT > 0  → pousse vers le jaune  |  B_SHIFT < 0 → pousse vers le bleu
A_SHIFT      = 0      # ajuster selon la teinte résiduelle après gray-world
B_SHIFT      = 0      # ajuster selon la teinte résiduelle après gray-world
RED_STRENGTH = 40     # force de la restauration du canal rouge (0–100)
CLAHE_CLIP   = 2.0    # clip limit CLAHE (2.0 = bon compromis contraste/bruit)
CLAHE_TILE   = (8, 8) # taille des tuiles CLAHE
# -------------------- Load Delegate --------------------
...
def clahe_enhance(img):
    lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
    L, A, B = cv2.split(lab)
    clahe = cv2.createCLAHE(clipLimit=max(CLAHE_CLIP, 0.1), tileGridSize=CLAHE_TILE)
    L2 = clahe.apply(L)
...
while True:
    ...
    frame_rs = preprocess_underwater(frame_rs)  # BUG CORRIGÉ : appel manquant
