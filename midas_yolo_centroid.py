...
UDP_IP           = "127.0.0.1"
UDP_PORT         = 17001
SELECT_PORT      = 17002          # écoute des sélections (Mac -> ROV)
LIST_PORT        = 17003          # diffusion de la liste d'objets (ROV -> Mac)
VIDEO_IN      = "12185607-hd_1920_1080_24fps.mp4"
...
# -------------------- Paramètres tracker --------------------
MAX_DIST_PIXELS  = 400 # distance max entre deux frames pour même objet
...
#selection socket, this doesn't break the loop 
...
sel_obj = None # selected object

def poll_selection(): #polling server 
    global selected_obj
    while True:
        try:
...
while True:
    ...
    poll_selection()        #update tracked obj 
    list_data=[]
    ...
    if len(boxes):
        ...
        if len(idx_cv2):
            ...
            for local_idx, i in enumerate(indx):
                ...
                # UDP avec valeurs filtrées
                x_out = tracked_objects[tid]['x_f']
                y_out = tracked_objects[tid]['y_f']
                z_out = tracked_objects[tid]['z_f']
                # Accumulate for UDP
                objects_data.append(
