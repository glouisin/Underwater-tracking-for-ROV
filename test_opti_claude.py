...
FRAME_W       = 640   # = resolution native camera (plus d'upscale)
FRAME_H       = 480
FPS_OUT       = 20    # cadence flux video (encode) — baisser = moins de chaleur
INFER_FPS     = 15    # cadence YOLO/MiDaS — le tracker comble entre 2 detections
CONF_THRES    = 0.25
...
# -------------------- Main Loop --------------------
# DECOUPLAGE :
#   - Traitement LOURD (YOLO/MiDaS + tracking + UDP->PID) : seulement sur frame NEUVE
#   - Flux VIDEO : cadence reguliere FPS_OUT, independante du rythme de l'IA
last_seq        = -1
loop_cnt        = 0
last_annotated  = None            # derniere image annotee, reutilisee entre 2 frames neuves
PUSH_DT         = 1.0 / FPS_OUT   # periode cible entre 2 push video
t_last_push     = 0.0
push_cnt        = 0
INFER_DT        = 1.0 / INFER_FPS # periode min entre 2 inferences (thermique)
t_last_detect   = 0.0
...
while True:
    ...
    # =====================================================================
    #  BLOC LOURD  ->  UNIQUEMENT sur une frame NEUVE de la camera
    # =====================================================================
    did_infer = False
    # Bloc lourd : frame neuve ET assez de temps ecoule depuis la derniere inference
    if cap.seq != last_seq and (time.time() - t_last_detect) >= INFER_DT:
        ...
        # ---- YOLO : filtrage en int8 AVANT dequant (evite de dequantifier 705k valeurs) ----
        raw     = output_raw_yolo              # [84, 8400] int8, pas de copie
        cls_raw = raw[4:, :]                   # [80, 8400] scores quantifies
        obj_raw = cls_raw.max(axis=0)          # [8400] meilleur score par boite (int8)
        thr_q   = CONF_THRES / out_scale_yolo + out_zp_yolo   # seuil dans l'espace int8
        keep    = obj_raw > thr_q              # masque ~8400 -> cheap

        sub = raw[:, keep]                     # [84, Nkeep] : uniquement les survivants
        if sub.shape[1]:
            det          = (sub.astype(np.float32) - out_zp_yolo) * out_scale_yolo
            det          = det.transpose()     # [Nkeep, 84]
            boxes        = det[:, :4]
            class_scores = det[:, 4:]
        ...
        if frame_cnt % 30 == 0:
            ...
            print(f"  Loop tour/s         : {loop_cnt / elapsed_loop:.1f}")   # tours (eleve)
            print(f"  FPS neuves/s        : {frame_cnt / elapsed_loop:.1f}")  # frames neuves = vrai FPS
            print(f"  FPS push video/s    : {push_cnt / elapsed_loop:.1f}")   # cadence du flux
        ...
        # ---------- Overlay fixe (centre) — dessine une seule fois par frame neuve ----------
        ...
        # Image annotee prete -> servira au flux video (y compris pour les push intermediaires)
        last_annotated = output_frame.copy()

        # ---------- Envois UDP (sur frame NEUVE uniquement) ----------
    ...
    # =====================================================================
    #  FLUX VIDEO  ->  cadence reguliere FPS_OUT, decouplee de l'IA
    #  (re-pousse la derniere image annotee pour garder un flux fluide
    #   meme entre 2 frames neuves)
    # =====================================================================
    now = time.time()
    if last_annotated is not None and (now - t_last_push) >= PUSH_DT:
        t_last_push = now
        push_cnt   += 1
        data = last_annotated.tobytes()
        buf  = Gst.Buffer.new_wrapped(data)         # une copie en moins (vs new_allocate + fill)
        buf.duration = Gst.util_uint64_scale_int(1, Gst.SECOND, FPS_OUT)
        buf.pts = buf.dts = push_cnt * buf.duration
        appsrc.emit('push-buffer', buf)

    # anti busy-spin : si pas d'inference cette iteration, on dort un poil
    if not did_infer:
        time.sleep(0.001)
