...
z_desired = None
z_desired_pending = False   # AJOUT : capture différée à la première vraie mesure
    ...
    while True:
        ...
        if msg == "clear":
            ...
            try:
                ...
                selected_id = new_id
                z_desired = None
                z_desired_pending = True   # capture différée, voir boucle principale
        ...
        if len(idx_cv2):
            ...
            for local_idx, i in enumerate(indx):
                ...
                if depth_map is not None:
                    ...
                    z_raw = 0.0
                z_corrected = Z_MAX - z_raw  # large = far, small = close — toujours calculé, pour l'affichage
                tid = track_assignments[local_idx]
                if depth_map is not None:
                    # Ne met à jour z_f QUE si une vraie mesure de profondeur existe
                    # cette frame — évite de contaminer l'EMA avec le placeholder
                    # utilisé avant sélection (quand depth_map est None).
                    ...
                    # Capture différée de z_desired : première vraie mesure de
                    # profondeur après une sélection, ARRONDIE à la centaine
                    # supérieure pour garder une marge de sécurité (avec
                    # z_corrected: large=far/small=close, arrondir vers le haut
                    # = viser toujours un peu plus loin que la mesure réelle).
                   
                    if z_desired_pending and tid == selected_id:
                        raw_z = tracked_objects[tid]['z_f']
                        z_desired = math.ceil(raw_z / 100.0) * 100.0
                        print(f"[Z_DESIRED] Mesure : {raw_z:.1f} -> setpoint arrondi : {z_desired:.0f}")
                        z_desired_pending = False
